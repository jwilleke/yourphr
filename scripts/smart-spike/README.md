...
Runs the full standalone patient-launch authorization-code + PKCE flow against a SMART __sandbox__ using the same libraries the production backend will use (`golang.org/x/oauth2` + `net/http`), then fetches `Patient/$everything` and saves the Bundle. It de-risks the architecture before we build the real client and relay (#49–#53).

This is a __separate Go module__ (its own `go.mod`) so it never affects the backend build or `make test-backend`.

## Safety

Point this __only__ at a sandbox with synthetic test patients — never a real provider. No PHI. The default target is the SMART Health IT R4 sandbox, which accepts public clients and a loopback redirect, so __no app registration and no relay are needed__ for the spike.
