...
export const US_CORE_CONFORMANCE: UsCoreProfileConformance[] = [
  ...
  // ------------------------------------------------------ Condition (Problems & Health Concerns)
  {
    ...
    mustSupport: [
      { path: 'Condition.meta.lastUpdated', label: 'meta.lastUpdated', status: 'gap',
        note: 'Not surfaced — the Condition card does not show the record last-updated timestamp.',
        present: (r) => !!r.meta?.lastUpdated, surfaced: (m) => !!m.meta_last_updated },
      { path: 'Condition.extension:assertedDate', label: 'assertedDate extension', status: 'gap',
        note: 'Asserted-date extension is not surfaced as its own element (only used as an onset fallback).',
  ...
  // -------------------------------------------------------------------- MedicationRequest
  {
    ...
    mustSupport: [
      ...
      { path: 'MedicationRequest.dispenseRequest', label: 'dispenseRequest', status: 'gap',
        note: 'Not surfaced — the MedicationRequest model does not expose dispenseRequest (quantity/refills).',
        present: (r) => !!r.dispenseRequest, surfaced: (m) => !!m.dispense_request },
    ],
  },
  ...
  // ------------------------------------------------------------ Observation (Laboratory Result)
  {
    ...
    mustSupport: [
      { path: 'Observation.meta.lastUpdated', label: 'meta.lastUpdated', status: 'gap',
        note: 'Not surfaced — the Observation card does not show the record last-updated timestamp.',
        present: (r) => !!r.meta?.lastUpdated, surfaced: (m) => !!m.meta_last_updated },
      ...
      { path: 'Observation.specimen', label: 'specimen', status: 'gap',
        note: 'Not surfaced — the Observation model does not expose the specimen reference.',
  ...
  // ------------------------------------------------------------------- DocumentReference
  {
    ...
    mustSupport: [
      ...
      { path: 'DocumentReference.context.encounter', label: 'context.encounter', status: 'gap',
        note: 'Not surfaced — the model context omits the encounter reference (only event/facility/period).',
