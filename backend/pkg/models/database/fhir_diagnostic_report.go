...
func (s *FhirDiagnosticReport) PopulateAndExtractSearchParameters(resourceRaw json.RawMessage) error {
	...
	sortTitleResult, err := vm.RunString("(function(){var c=fhirResource.code;if(!c)return undefined;if(c.text)return c.text;if(c.coding&&c.coding[0]&&c.coding[0].display)return c.coding[0].display;return undefined;})()")
	...
	sortDateResult, err := vm.RunString("(function(){var r=fhirResource;if(r.effectiveDateTime)return r.effectiveDateTime;if(r.effectivePeriod&&r.effectivePeriod.start)return r.effectivePeriod.start;return undefined;})()")
