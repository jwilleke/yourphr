...
func (s *FhirInsurancePlan) PopulateAndExtractSearchParameters(resourceRaw json.RawMessage) error {
	...
	// extracting Address
	addressResult, err := vm.RunString("extractStringSearchParameters(fhirResource, 'InsurancePlan.contact.address')")
	if err == nil && addressResult.String() != "undefined" {
		s.Address = []byte(addressResult.String())
	}
	// extracting AddressCity
	addressCityResult, err := vm.RunString("extractStringSearchParameters(fhirResource, 'InsurancePlan.contact.address.city')")
	if err == nil && addressCityResult.String() != "undefined" {
		s.AddressCity = []byte(addressCityResult.String())
	}
	// extracting AddressCountry
	addressCountryResult, err := vm.RunString("extractStringSearchParameters(fhirResource, 'InsurancePlan.contact.address.country')")
	...
	addressPostalcodeResult, err := vm.RunString("extractStringSearchParameters(fhirResource, 'InsurancePlan.contact.address.postalCode')")
	...
	addressStateResult, err := vm.RunString("extractStringSearchParameters(fhirResource, 'InsurancePlan.contact.address.state')")
	if err == nil && addressStateResult.String() != "undefined" {
		s.AddressState = []byte(addressStateResult.String())
	}
	// extracting AddressUse
	addressUseResult, err := vm.RunString("extractTokenSearchParameters(fhirResource, 'InsurancePlan.contact.address.use')")
	if err == nil && addressUseResult.String() != "undefined" {
		s.AddressUse = []byte(addressUseResult.String())
	}
	// extracting AdministeredBy
	administeredByResult, err := vm.RunString("extractReferenceSearchParameters(fhirResource, 'InsurancePlan.administeredBy')")
	if err == nil && administeredByResult.String() != "undefined" {
		s.AdministeredBy = []byte(administeredByResult.String())
	}
	// extracting Endpoint
	endpointResult, err := vm.RunString("extractReferenceSearchParameters(fhirResource, 'InsurancePlan.endpoint')")
	if err == nil && endpointResult.String() != "undefined" {
		s.Endpoint = []byte(endpointResult.String())
	}
	// extracting Identifier
	identifierResult, err := vm.RunString("extractTokenSearchParameters(fhirResource, 'InsurancePlan.identifier')")
	...
	// extracting Name
	nameResult, err := vm.RunString("extractStringSearchParameters(fhirResource, 'name | alias')")
	...
	// extracting OwnedBy
	ownedByResult, err := vm.RunString("extractReferenceSearchParameters(fhirResource, 'InsurancePlan.ownedBy')")
	if err == nil && ownedByResult.String() != "undefined" {
		s.OwnedBy = []byte(ownedByResult.String())
	}
	// extracting Phonetic
	phoneticResult, err := vm.RunString("extractStringSearchParameters(fhirResource, 'InsurancePlan.name')")
	if err == nil && phoneticResult.String() != "undefined" {
		s.Phonetic = []byte(phoneticResult.String())
	}
	// extracting Status
	statusResult, err := vm.RunString("extractTokenSearchParameters(fhirResource, 'InsurancePlan.status')")
