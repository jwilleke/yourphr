...
func (s *FhirProcedure) PopulateAndExtractSearchParameters(resourceRaw json.RawMessage) error {
	...
	sortDateResult, err := vm.RunString("(function(){var r=fhirResource;if(r.performedDateTime)return r.performedDateTime;if(r.performedPeriod&&r.performedPeriod.start)return r.performedPeriod.start;return undefined;})()")
