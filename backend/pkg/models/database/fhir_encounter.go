...
func (s *FhirEncounter) PopulateAndExtractSearchParameters(resourceRaw json.RawMessage) error {
	...
	sortTitleResult, err := vm.RunString("(function(){var r=fhirResource;if(r.type&&r.type[0]){var t=r.type[0];if(t.text)return t.text;if(t.coding&&t.coding[0]&&t.coding[0].display)return t.coding[0].display;}if(r.serviceType){var st=r.serviceType;if(st.text)return st.text;if(st.coding&&st.coding[0]&&st.coding[0].display)return st.coding[0].display;}if(r.location&&r.location[0]&&r.location[0].location&&r.location[0].location.display)return r.location[0].location.display;return undefined;})()")
	...
	sortDateResult, err := vm.RunString("(function(){var r=fhirResource;if(r.period&&r.period.start)return r.period.start;return undefined;})()")
