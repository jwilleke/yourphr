...
// validateConfig fails fast with a clear error when search.enabled is true but a setting it
// depends on is missing, rather than letting initClient time out after 30 retries against an
// api_key-less or uri-less Typesense connection.
func validateConfig(cfg config.Interface) error {
	if cfg.GetString("search.uri") == "" {
		return errors.New("search.uri cannot be empty when search.enabled is true")
	}
	if cfg.GetString("search.api_key") == "" {
		return errors.New("search.api_key cannot be empty when search.enabled is true")
	}
	if cfg.GetString("search.collection_name") == "" {
		return errors.New("search.collection_name cannot be empty when search.enabled is true")
	}
	if cfg.GetString("search.chat.conversation_collection_name") != "" {
		if cfg.GetString("search.chat.model.id") == "" {
			return errors.New("search.chat.model.id cannot be empty when search.chat.conversation_collection_name is set")
		}
		if cfg.GetString("search.chat.model.name") == "" {
			return errors.New("search.chat.model.name cannot be empty when search.chat.conversation_collection_name is set")
		}
		if cfg.GetString("search.chat.model.vllm_url") == "" {
			return errors.New("search.chat.model.vllm_url cannot be empty when search.chat.conversation_collection_name is set")
		}
	}
	return nil
}
