...
func (suite *SqliteRepositoryTestSuite) TestNewSqliteRepository_WithEncryptionKey_WhenReopenWithIncorrectKeyShouldFail() {
	...
	fakeConfig1.EXPECT().GetBool("search.enabled").Return(false).AnyTimes()
	...
	fakeConfig2.EXPECT().GetBool("search.enabled").Return(false).AnyTimes()
...
func (suite *SqliteRepositoryTestSuite) TestNewSqliteRepository_WithSearchEnabled() {
	...
	fakeConfig.EXPECT().GetString("database.type").Return("sqlite").AnyTimes()
	fakeConfig.EXPECT().IsSet("database.encryption.key").Return(false).AnyTimes()
	fakeConfig.EXPECT().GetBool("search.enabled").Return(true).AnyTimes()
	fakeConfig.EXPECT().GetString("log.level").Return("INFO").AnyTimes()
	fakeConfig.EXPECT().GetBool("database.validation_mode").Return(false).AnyTimes()
	fakeConfig.EXPECT().GetBool("database.encryption.enabled").Return(false).AnyTimes()
	fakeConfig.EXPECT().GetBool("search.enabled").Return(false).AnyTimes()
