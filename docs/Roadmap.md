...
| Item | Upstream ref | Notes |
|---|---|---|
| **OIDC / SSO login** | [PR #613](https://github.com/fastenhealth/fasten-onprem/pull/613) | Native OIDC (Google, Auth0, Authentik, Okta) alongside username/password. Could replace Authentik forward-auth middleware. ~23 files, 16 commits. |
...
Replace the commercial Fasten Lighthouse with a self-hosted Cloudflare Worker relay. Full plan: [`docs/oauth-gateway.md`](oauth-gateway.md).

| Item | Notes |
|---|---|
| **Cloudflare Worker OAuth relay** | Public callback endpoint; stores code in KV (60s TTL); Fasten polls and exchanges locally. See `docs/oauth-gateway.md`. |
