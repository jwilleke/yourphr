...
- **Oracle/Cerner** — public/PKCE. Cerner serves ONE authorize endpoint per FHIR base via discovery, and its **persona** (patient vs provider) is fixed by host/tenant. `fhir-myrecord` + the common tenant → `unknown-tenant`; `fhir-ehr-code` → provider persona, which a patient-registered `client_id` is rejected from (`client-persona-mismatch`). Connecting a Cerner _patient_ app needs either a provider app + provider login, or an authorize-endpoint override in the catalog. ([`oracle-cerner.md`](./oracle-cerner.md))
