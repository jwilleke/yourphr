...
| `could not retrieve authorization code from relay: timed out` **and the popup said "Connected"** | The login **took longer than YourPHR's ~90s poll window** (backend polls the relay 30s × 3 frontend retries). The popup "Connected" means the relay *did* get the code — but the backend had already stopped polling. Not a config error. | Retry **may** help if your login finishes within ~90s — but if it keeps timing out, the window is just too short for this provider. **Known limitation; the fix is to extend the poll window.** Workaround: pre-log-in at `bluebutton.cms.gov` in another tab first, then Connect so the popup skips straight to Authorize. |
